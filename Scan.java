...
//             'hello' is returned from the input 'hello*45'?
            ...
            else if (myisstring((char) c))
				return new Token(TK.STR, buildSTR(), linenumber);
            else {
                switch( c ) {
                    case ',':
...
// QUESTION 3:  What does the following case and the code in it do?
	...
	private String buildSTR() {
		...
		do {
			...
			c = getchar();
			err += (char) c;
			//System.out.println("Log: buildSTR: read in: " + (char)c);
		} while ((char)c != '"' && c != -1);
		putback = true;
		str += '"';
		...
		if (c != '"') {
			...
			c = getchar();
		//System.out.println("Log: created string: " + str);
		return str;
	}
    ...
    private static boolean myisstring(char c) {
		if (c == '"') return true;
		else return false;
	}
